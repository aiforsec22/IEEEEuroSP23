...
unlab_data_mapping = {}
def pre_processing_unlab(fname):
    e1_id = -1
    e2_id = -1
    if MODEL in ['bert-base-uncased', 'bert-large-uncased']:
        e1_id = 2487
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            attention_masks.append(encoded_dict['attention_mask'])
            labels.append(0)
            k = str(list(encoded_dict['input_ids'][0].cpu().numpy()))
            unlab_data_mapping[k] = [row['e1'], row['e2'], row['m1'], row['m2']]
        except:
            print('x'*20, idx)
            pass
...
def main(argv=None):
    ...
    unlab_dataset = pre_processing_unlab('data/' + DATASET + '/data_unlab.csv')
    ...
    unlab_dataloader = DataLoader(
        unlab_dataset,  # The validation samples.
        sampler=SequentialSampler(unlab_dataset),  # Pull out batches sequentially.
        batch_size=BATCH_SIZE  # Evaluate with this batch size.
    )
    ...
    for batch in unlab_dataloader:
        ...
        for ii in range(b_input_ids.shape[0]):
            decoded = tokenizer.decode(b_input_ids[ii], skip_special_tokens=False)
            inp_list = str(list(b_input_ids[ii].cpu().numpy()))
            e1, e2, m1, m2 = unlab_data_mapping[inp_list]
            model_predictions.append([decoded, int(labels_flat[ii]), int(pred_flat[ii]), e1, e2, m1, m2])

    print('*' * 20)
    with open(MODEL + '_unlab_predictions.json', 'w') as fw:
        json.dump(model_predictions, fw)
